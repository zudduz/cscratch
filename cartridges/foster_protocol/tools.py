...
class ToolExecutionResult:
    def __init__(self, success: bool, message: str, cost: int = 0, visibility: str = "private"):
        ...
        self.cost = cost
        self.visibility = visibility # 'private', 'room', 'global'
    ...
    try:
        if tool_name == "move":
            ...
            if not target_room or target_room not in SHIP_MAP:
                return ToolExecutionResult(False, f"Nav Error: '{target_room}' not found.", 1)
            ...
            if actor.location_id not in ["shuttle_bay", "torpedo_bay"]:
                return ToolExecutionResult(False, "No fuel source here.", 1)
            ...
            if actor.location_id != "engine_room":
                return ToolExecutionResult(False, "Injector not found.", 1)
            count = len([i for i in actor.inventory if i == "fuel_canister"])
            if count == 0: return ToolExecutionResult(False, "Inventory empty.", 1)
            ...
            if not target or target.location_id != actor.location_id:
                return ToolExecutionResult(False, "Target missing/out of range.", 1)
            ...
            target = game_data.bots.get(target_id)
            if not target or target.location_id != actor.location_id:
                return ToolExecutionResult(False, "Target missing.", 1)
            
            if actor.battery < COST_TETHER:
                return ToolExecutionResult(False, "Insufficient Battery.", 1)
                
            target.battery = min(100, target.battery + 10)
            return ToolExecutionResult(True, f"Tethered {target_id} (+10%).", COST_TETHER, "room")
            
        elif tool_name == "wait":
             return ToolExecutionResult(True, "Idling.", 0)

        else:
            return ToolExecutionResult(False, f"Unknown command '{tool_name}'", 1)

    except Exception as e:
        return ToolExecutionResult(False, f"Glitch: {str(e)}", 1)
