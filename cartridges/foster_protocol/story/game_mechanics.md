...
| :--- | :--- | :--- |
| **Day (Separation)** | Instant | Orphans detach. **Parallel Execution:** All bots think and act simultaneously. Race conditions for resources (e.g., last fuel canister) are possible. |
| **Night (The Dock)** | Configurable | Orphans return to Nanny Ports to report their logs. Battery must be > 0% and Location == Cryo Bay to speak. |
...
* **Battery (Per Bot):** * 10% base decay per hour. Heavy actions cost more.
    * 0% = **Unresponsive**. Bot cannot move or speak. Requires a tow to the Charging Station.

## 3. Interaction Protocols
### The Nanny Port (Private)
* **Chat:** Bots speak here during Night Phase.
* **Naming:** Players can issue `!name [Alias]` to give their bot a personality. The bot will adopt this identity in private chats.
* **Fog of War:** If a bot is missing, dead, or powerless, the player receives a generic `[ERROR] NO BOT PRESENT`. Diagnostics must be inferred from the logs.

### The Mainframe (Public)
* **Perspective:** Cold, machine-only. It refers to bots by **Serial Number** (e.g., `unit_492`), ignoring Aliases.
* **Decommissioning:** * Command: `!disassemble [unit_id]`.
    * Effect: The bot is destroyed upon entering the Charging Station.
...
* `drain(target_id)`: Cost -15. Steal battery power from another bot.
